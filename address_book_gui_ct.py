...
# -------------------------- CREATE FRAMES --------------------------------#
    def create_frames(self):
        self.entry_frame = ct.CTkFrame(
            master=self
            # relief=ct.GROOVE
        )
        self.operations_frame = ct.CTkFrame(
            master=self
            # relief=ct.GROOVE
        )
        self.treeview_frame = ct.CTkFrame(
            master=self
            # relief=ct.GROOVE
        )
        # Grid the frames
        self.entry_frame.grid(row=0, column=0, sticky=ct.NW)
        self.operations_frame.grid(row=0, column=1, sticky=ct.N)
        self.treeview_frame.grid(row=1, column=0, columnspan=2, sticky=ct.W)

# ------------------------- CREATE WIDGETS ------------------------------- #
    def create_widgets(self):
        # ------------------------ CREATE LABELS ------------------------- #
        self.lbl_first_name = ct.CTkLabel(
            self.entry_frame, text="First Name:", anchor="e")
        self.lbl_last_name = ct.CTkLabel(
            self.entry_frame, text="Last Name:", anchor="e")
        self.lbl_phone = ct.CTkLabel(
            self.entry_frame, text="Phone:", anchor="e")
        self.lbl_email = ct.CTkLabel(
        ...
        self.entry_lname = ct.CTkEntry(self.entry_frame, width=ENTRY_WIDTH)
        self.entry_phone = ct.CTkEntry(self.entry_frame, width=ENTRY_WIDTH)
        self.entry_email = ct.CTkEntry(self.entry_frame, width=ENTRY_WIDTH)

        # ------------------------ CREATE BUTTONS ------------------------ #
        self.btn_add = ct.CTkButton(
            self.operations_frame,
            text="Add",
            command=self.insert_record
        )
        self.btn_modify = ct.CTkButton(
            self.operations_frame,
            text="Update Selected",
            command=self.update_record
        )
        self.btn_delete = ct.CTkButton(
            self.operations_frame,
            text="Delete Selected",
            command=self.delete_record
        )
        self.btn_close = ct.CTkButton(
            self.operations_frame,
            text="Close",
            command=self.close
        )
        ...
        self.btn_add.grid(row=0, column=0, sticky=ct.EW)
        self.btn_modify.grid(row=1, column=0, sticky=ct.EW)
        self.btn_delete.grid(row=2, column=0, sticky=ct.EW)
        self.btn_close.grid(row=3, column=0, sticky=ct.EW)
...
# ------------------------- TREEVIEW AND SCROLLBAR ----------------------- #
    def create_treeview(self):
        ...
        bg_color = self._apply_appearance_mode(
            ct.ThemeManager.theme["CTkFrame"]["fg_color"])
        text_color = self._apply_appearance_mode(
            ct.ThemeManager.theme["CTkLabel"]["text_color"])
        selected_color = self._apply_appearance_mode(
            ct.ThemeManager.theme["CTkButton"]["fg_color"])
        ...
        treestyle.configure("Treeview.Heading", font=(None, 14))
        treestyle.configure("Treeview", font=(None, 14), rowheight=30)
        ...
        self.tree.heading("first_name", text="First Name", anchor=ct.W)
        self.tree.heading("last_name", text="Last Name", anchor=ct.W)
        self.tree.heading("phone", text="Phone", anchor=ct.W)
        self.tree.heading("email", text="Email", anchor=ct.W)
...
# --------------------- CLEAR ENTRY WIDGETS ------------------------------ #
    def clear_entry_widgets(self):
        # Clear entry widgets, set focus to name entry widget
        self.entry_fname.delete(0, ct.END)
        self.entry_lname.delete(0, ct.END)
        self.entry_phone.delete(0, ct.END)
        self.entry_email.delete(0, ct.END)
        self.entry_fname.focus()
